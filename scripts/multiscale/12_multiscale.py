# Multiscale Experiments for FWI SVGD
import warnings
...
@hydra.main(config_path="../../config/multiscale", config_name="12_multiscale")
def main(cfg):
    ...
    step=0
    frequencies=np.array(cfg.params.peak_freq)
    ...
    # start SVGD
    print(f"Starting multiscale inversion with {cfg.params.num_particles} particles")
    t_start = time.time()
    ...
    # learning_rates=np.array([100,10,10])
    
    for step in range(frequencies.size):
        
        # Initialize the  kernel 
        K = RBF()
        
        # Set the optimizer
        optimizer = torch.optim.Adam([X_init],lr=cfg.params.learning_rate[step])
        # print('Learning rate '+str(cfg.params.learning_rate[step])+'')
        ...
        for iteration in epochs:
            ...
            if iteration % 20 == 0 and iteration != 0 and iteration <= (cfg.params.num_iterations - 50):
                replaced_particles = replace_particles(kernel_matrix=svgd.K_XX.detach().cpu().numpy(), 
                                          particle_data=X_init.detach().clone().cpu(), 
                                          threshold=1e-2, percentage=80, 
                                          device='cpu', kind='interacting')
                with torch.no_grad():
            ...
            if iteration % 100 == 0:
                ...
                plot_update_models(results_svgd["updates_mean"], results_svgd["updates_std"], 
                                ...
                                save_plots=True, 
                                mean_plot_filename=f"{fig_path}mean{iteration}_{freq}.png", 
                                std_plot_filename=f"{fig_path}std_{iteration}_{freq}.png",
                                kernel_plot_filename=f"{fig_path}kernel_{iteration}_{freq}.png")


            # Update tqdm postfix
            epochs.set_postfix(iter=iteration, loss=running_loss, lr=cfg.params.learning_rate[step], freq=freq)
    ...
    #Save the dictionary 
    save_dict_as_compressed_npz(f"{cfg.paths.out_path}12_results_svgd.npz", results_svgd)
