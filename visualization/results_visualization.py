...
exp_id = f"{15:02d}"
print(f'Starting plots - file: exp_{exp_id} ...')

with initialize(version_base=None, config_path="../config/exp"):
    cfg = compose(f"{exp_id}_exp")
...
out_path = f'../videos/exp/{exp_id}_exp/'
temp_path = f'../temp_frames/exp/{exp_id}_exp/'
...
plt.tight_layout()
plt.savefig(f'{out_path}{exp_id}_marginal_pseudo_wells.png', dpi=150)
...
plt.tight_layout()
plt.savefig(f'{out_path}{exp_id}_marginal_pixels.png', dpi=150)
...
# extent_x =[0,(nx-1)*dx*1e-3]
# extent_z =[(nz-1)*dx*1e-3,0]

# plot_multiple_means_stds(global_mean, global_std, mean_cluster_0, std_cluster_0, mean_cluster_1, std_cluster_1, 
#                             cfg, m_vmin, m_vmax, std_min, std_max, extent_x, extent_z, plot_path, save_plots=True)
